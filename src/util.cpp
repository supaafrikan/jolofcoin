...
const char * const BITCOIN_CONF_FILENAME = "jolofcoin.conf";
const char * const BITCOIN_PID_FILENAME = "jolofcoin.pid";
...
#else
    const char* pszModule = "jolofcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Jolofcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Jolofcoin";
#else
    // Unix
    return pathRet / ".jolofcoin";
#endif
#endif
}
